...
def generate_backscatter_echoes(echo={"loc":20,"scale":5,"samples":10}, 
        noise={"loc":2,"scale":1,"samples":10}, seed=0, dur=120, beams=range(4,24), 
        base_dir="../data/raw/", fname="dat_%04d_%03d.txt", clean=True):
...
def generate_backscatter_damp_function(dur=120, seed=0, base_dir="../data/damp/", fname="dmp_dat_%03d_%.2f_%.2f_%.2f.txt", clean=True):
    if clean: os.system("rm -rf " + base_dir + "*")
    np.random.seed(seed)
    flare_time_location_distribution = np.random.uniform(10, 90, 100).astype(int)
    percentage_drop_ditsribution = np.round(np.random.uniform(0.5, 1., 10), 2)
    drop_rate_distribution = np.round(np.random.normal(3, .5, 10), 2)
    gain_rate_distribution = np.round(np.random.uniform(5e-2, 5e-1, 10), 2)
